#!/usr/bin/env python3
"""Generate Spawner Shards' item sprite: three pieces of a broken spawner cage.

The palette is not written down here, it is read out of block/spawner.png and
sorted by brightness, so the shards are the cage's own colours and stay its
colours if Mojang ever repaints it. Only the shapes are ours.
...
OUT = os.path.join(HERE, "src/main/resources/assets/spawner-shards-justfatlard/textures/item/spawner_shards.png")
...
def build_item():
    return draw_shards([[CLEAR] * 16 for _ in range(16)])
